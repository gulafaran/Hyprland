    ...
    try {
        ...
        if (!*PCM)
            ...
            if (m_cmSupported) {
                ...
                createShaderVao(shaders->m_shCM);
            } else
                Debug::log(ERR,
        ...
        createShaderVao(shaders->m_shQUAD);
        ...
        createShaderVao(shaders->m_shRGBA);
        ...
        createShaderVao(shaders->m_shPASSTHRURGBA);
        ...
        createShaderVao(shaders->m_shMATTE);
        ...
        createShaderVao(shaders->m_shGLITCH);
        ...
        createShaderVao(shaders->m_shRGBX);
        ...
        createShaderVao(shaders->m_shEXT);
        ...
        createShaderVao(shaders->m_shBLUR1);
        ...
        createShaderVao(shaders->m_shBLUR2);
        ...
        createShaderVao(shaders->m_shBLURPREPARE);
        ...
        createShaderVao(shaders->m_shBLURFINISH);
        ...
        createShaderVao(shaders->m_shSHADOW);
        ...
        createShaderVao(shaders->m_shBORDER1);
...
void CHyprOpenGLImpl::createShaderVao(SShader& shader) {
    glGenVertexArrays(1, &shader.shaderVao);
    glBindVertexArray(shader.shaderVao);

    if (shader.posAttrib != -1) {
        glGenBuffers(1, &shader.shaderVboPos);
        glBindBuffer(GL_ARRAY_BUFFER, shader.shaderVboPos);
        glBufferData(GL_ARRAY_BUFFER, sizeof(fullVerts), fullVerts, GL_STATIC_DRAW);
        glEnableVertexAttribArray(shader.posAttrib);
        glVertexAttribPointer(shader.posAttrib, 2, GL_FLOAT, GL_FALSE, 0, (void*)0);
    }

    // UV VBO (dynamic, may be updated per frame)
    if (shader.texAttrib != -1) {
        glGenBuffers(1, &shader.shaderVboUv);
        glBindBuffer(GL_ARRAY_BUFFER, shader.shaderVboUv);
        glBufferData(GL_ARRAY_BUFFER, sizeof(fullVerts), fullVerts, GL_DYNAMIC_DRAW); // Initial dummy UVs
        glEnableVertexAttribArray(shader.texAttrib);
        glVertexAttribPointer(shader.texAttrib, 2, GL_FLOAT, GL_FALSE, 0, (void*)0);
    }
